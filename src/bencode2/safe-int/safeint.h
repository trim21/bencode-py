...
/* For maximum portability include the exact-int module from
   portable snippets. */
#if !defined(psnip_int64_t) || !defined(psnip_uint64_t) || !defined(psnip_int32_t) ||              \
    !defined(psnip_uint32_t) || !defined(psnip_int16_t) || !defined(psnip_uint16_t) ||             \
...
#define PSNIP_SAFE_DEFINE_LARGER_BINARY_OP(T, name, op_name, op)                                   \
    PSNIP_SAFE__FUNCTION psnip_safe_##name##_larger psnip_safe_larger_##name##_##op_name(T a,      \
                                                                                         T b) {    \
...
#define PSNIP_SAFE_DEFINE_LARGER_UNARY_OP(T, name, op_name, op)                                    \
    PSNIP_SAFE__FUNCTION psnip_safe_##name##_larger psnip_safe_larger_##name##_##op_name(          \
        T value) {                                                                                 \
...
#define PSNIP_SAFE_DEFINE_BUILTIN_BINARY_OP(T, name, op_name)                                      \
    PSNIP_SAFE__FUNCTION psnip_safe_bool psnip_safe_##name##_##op_name(T *res, T a, T b) {         \
...
#define PSNIP_SAFE_DEFINE_SIGNED_ADD(T, name, min, max)                                            \
    PSNIP_SAFE__FUNCTION psnip_safe_bool psnip_safe_##name##_add(T *res, T a, T b) {               \
        psnip_safe_bool r = !(((b > 0) && (a > (max - b))) || ((b < 0) && (a < (min - b))));       \
...
#define PSNIP_SAFE_DEFINE_SIGNED_SUB(T, name, min, max)                                            \
    PSNIP_SAFE__FUNCTION psnip_safe_bool psnip_safe_##name##_sub(T *res, T a, T b) {               \
        psnip_safe_bool r = !((b > 0 && a < (min + b)) || (b < 0 && a > (max + b)));               \
        if (PSNIP_SAFE_LIKELY(r))                                                                  \
            *res = a - b;                                                                          \
        return r;                                                                                  \
    }
...
#define PSNIP_SAFE_DEFINE_SIGNED_MUL(T, name, min, max)                                            \
    PSNIP_SAFE__FUNCTION psnip_safe_bool psnip_safe_##name##_mul(T *res, T a, T b) {               \
        psnip_safe_bool r = 1;                                                                     \
        if (a > 0) {                                                                               \
            if (b > 0) {                                                                           \
                if (a > (max / b)) {                                                               \
...
#define PSNIP_SAFE_DEFINE_SIGNED_DIV(T, name, min, max)                                            \
    PSNIP_SAFE__FUNCTION psnip_safe_bool psnip_safe_##name##_div(T *res, T a, T b) {               \
...
#define PSNIP_SAFE_DEFINE_SIGNED_MOD(T, name, min, max)                                            \
    PSNIP_SAFE__FUNCTION psnip_safe_bool psnip_safe_##name##_mod(T *res, T a, T b) {               \
        if (PSNIP_SAFE_UNLIKELY(b == 0)) {                                                         \
            *res = 0;                                                                              \
            return 0;                                                                              \
        } else if (PSNIP_SAFE_UNLIKELY(a == min && b == -1)) {                                     \
...
#define PSNIP_SAFE_DEFINE_SIGNED_NEG(T, name, min, max)                                            \
    PSNIP_SAFE__FUNCTION psnip_safe_bool psnip_safe_##name##_neg(T *res, T value) {                \
...
#define PSNIP_SAFE_DEFINE_INTSAFE(T, name, op, isf)                                                \
    PSNIP_SAFE__FUNCTION psnip_safe_bool psnip_safe_##name##_##op(T *res, T a, T b) {              \
...
PSNIP_SAFE_DEFINE_PROMOTED_SIGNED_BINARY_OP(psnip_int32_t, int32, add, (-0x7fffffffLL - 1),
                                            0x7fffffffLL)
PSNIP_SAFE_DEFINE_PROMOTED_SIGNED_BINARY_OP(psnip_int32_t, int32, sub, (-0x7fffffffLL - 1),
                                            0x7fffffffLL)
PSNIP_SAFE_DEFINE_PROMOTED_SIGNED_BINARY_OP(psnip_int32_t, int32, mul, (-0x7fffffffLL - 1),
                                            0x7fffffffLL)
#else
...
PSNIP_SAFE_DEFINE_PROMOTED_SIGNED_BINARY_OP(psnip_int64_t, int64, add, (-0x7fffffffffffffffLL - 1),
                                            0x7fffffffffffffffLL)
PSNIP_SAFE_DEFINE_PROMOTED_SIGNED_BINARY_OP(psnip_int64_t, int64, sub, (-0x7fffffffffffffffLL - 1),
                                            0x7fffffffffffffffLL)
PSNIP_SAFE_DEFINE_PROMOTED_SIGNED_BINARY_OP(psnip_int64_t, int64, mul, (-0x7fffffffffffffffLL - 1),
                                            0x7fffffffffffffffLL)
#else
PSNIP_SAFE_DEFINE_SIGNED_ADD(psnip_int64_t, int64, (-0x7fffffffffffffffLL - 1),
                             0x7fffffffffffffffLL)
PSNIP_SAFE_DEFINE_SIGNED_SUB(psnip_int64_t, int64, (-0x7fffffffffffffffLL - 1),
                             0x7fffffffffffffffLL)
PSNIP_SAFE_DEFINE_SIGNED_MUL(psnip_int64_t, int64, (-0x7fffffffffffffffLL - 1),
                             0x7fffffffffffffffLL)
#endif
PSNIP_SAFE_DEFINE_SIGNED_DIV(psnip_int64_t, int64, (-0x7fffffffffffffffLL - 1),
                             0x7fffffffffffffffLL)
PSNIP_SAFE_DEFINE_SIGNED_MOD(psnip_int64_t, int64, (-0x7fffffffffffffffLL - 1),
                             0x7fffffffffffffffLL)
PSNIP_SAFE_DEFINE_SIGNED_NEG(psnip_int64_t, int64, (-0x7fffffffffffffffLL - 1),
                             0x7fffffffffffffffLL)
...
#define psnip_safe_neg(res, v) PSNIP_SAFE_C11_GENERIC_UNARY_OP(neg, res, v)
    ...
    (defined(PSNIP_SAFE_EMULATE_NATIVE) || defined(PSNIP_BUILTIN_EMULATE_NATIVE))
